...
class StatisticsKafkaResourceIT {
    ...
    void setup() {
        ...
        StatisticsKafkaResource kafkaResource = new StatisticsKafkaResource(kafkaProperties);
    ...
    void producesMessages() throws Exception {
        restMockMvc.perform(post("/api/statistics-kafka/publish/topic-produce?message=value-produce"))
    ...
    void consumesMessages() throws Exception {
        ...
        MvcResult mvcResult = restMockMvc.perform(get("/api/statistics-kafka/consume?topic=topic-consume"))
            .andExpect(status().isOk())
